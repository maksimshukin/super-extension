-- Исправляем функцию apply_promocode_with_subscription для правильной обработки типов данных
CREATE OR REPLACE FUNCTION apply_promocode_with_subscription(
    promocode_text TEXT,
    user_uuid UUID,
    plan_id TEXT  -- Изменяем тип с UUID на TEXT для совместимости
) RETURNS JSON AS $$
DECLARE
...
BEGIN
    ...
    -- Пытаемся преобразовать plan_id в UUID, если это возможно
    BEGIN
        plan_uuid := plan_id::UUID;
    EXCEPTION WHEN OTHERS THEN
        -- Если не удается преобразовать в UUID, ищем по id как INTEGER
        ...
        -- Создаем UUID для подписки (используем gen_random_uuid())
        plan_uuid := gen_random_uuid();
    END;
    
    -- Если plan_id успешно преобразован в UUID, ищем тариф по UUID
