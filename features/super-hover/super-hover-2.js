if (!window.SuperHoverArchitect) {
    // ✅ ИЗМЕНЕНИЕ: Переименовываем объект для соответствия новому стилю
    window.SuperHoverArchitect = {
        // --- METADATA ---
        toolName: 'Super Hover',
...
onLoad(panelWrapper) {
   console.log(`[${this.toolName}] Загрузка в панель...`);
   this.cacheDOMElements(panelWrapper);
   this.setupEventListeners();
   this.loadUserPresets();
   this.showManagerView();
   console.log(`[${this.toolName}] Инструмент успешно загружен.`);
},
        ...
        getHeaderHTML() {
            return `
                ...
                <div id="dbm-panel-header-content"></div>
            `;
        },
        ...
        cacheDOMElements(panelWrapper) {
            this.els.panel = panelWrapper;
            Object.assign(this.els, {
                header: panelWrapper.querySelector('#super-panel-header'),
                content: panelWrapper.querySelector('#super-panel-content'),
                footer: panelWrapper.querySelector('#super-panel-footer'),
                headerContent: panelWrapper.querySelector('#dbm-panel-header-content'),
                managerView: panelWrapper.querySelector('#dbm-manager-view'),
                editorView: panelWrapper.querySelector('#dbm-editor-view'),
                effectsList: panelWrapper.querySelector('#dbm-effects-list'),
                addNewBtn: panelWrapper.querySelector('#dbm-add-new-effect-btn'),
                copyCodeBtn: panelWrapper.querySelector('#dbm-copy-code-btn'),
                closePanelBtn: panelWrapper.querySelector('#dbm-close-panel-btn'),
                backToManagerBtn: panelWrapper.querySelector('#dbm-back-to-manager-btn')
                // ... добавьте сюда остальные this.els по аналогии, используя panelWrapper.querySelector
            });
            // Глобальные элементы, которые находятся вне основной панели
            this.els.inspector = document.getElementById('super-inspector-tooltip');
            this.els.dynamicS
            ...
            setupEventListeners() {
                ...
                // ✅ КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: Кнопка закрытия теперь вызывает общий менеджер
                this.els.closePanelBtn.addEventListener('click', () => window.SuperPanelManager.hidePanel());
    
                // Глобальные обработчики остаются привязанными к document.body
                ...
                document.body.addEventListener('keydown', e => {
                    if (e.key === 'Escape') {
                        ...
                        window.SuperPanelManager.hideModal(); // Используем общий менеджер
                    }
                }, true);
            },
            ...
            getUniqueSelector(el, useBlockPrefix = true) {
                return window.SuperUtils.getUniqueSelector(el, useBlockPrefix);
            },
            getSelectorPart(el) {
                return window.SuperUtils.getSelectorPart(el);
            },
            cleanSelectorForDisplay(selector) {
                return window.SuperUtils.cleanSelectorForDisplay(selector);
            },
            getBlockInfo(selector) {
                return window.SuperUtils.getBlockInfo(selector);
            },
            ...
            showModalConfirm(title, text) {
                return new Promise(resolve => {
                    ...
                    window.SuperPanelManager.showModal(modalHTML);
                    
                    const cleanup = (val) => { window.SuperPanelManager.hideModal(); resolve(val); };
            ...
            removeModal() {
                window.SuperPanelManager.hideModal();
            },
...
dbmGetControlsHTML() {
        ...
        dbmUpdateHeader(title, isEditor = false, subtitle = '') {
            const headerContent = this.els.panel.querySelector('#dbm-panel-header-content');
...
activateEditorFor(tabItem) {
    ...
    this.updateDynamicButtons(); // ✅ ИСПРАВЛЕНИЕ: Вызываем новую единую функцию
        ...
        getTranslationForSelector(selector) { return window.SuperUtils.TRANSLATION_MAP[selector] || ''; },
        getBlockInfo(selector) {
       ...
       getDefaultEffectState: () => ({ parent: window.SuperHoverArchitect.getDefaultState(), children: {}, _hasEffects: [], disableOnMobile: true }),
       getDefaultState: () => ({
        // Animation
...
window.SuperHoverArchitect = window.dbmHoverArchitect;
}